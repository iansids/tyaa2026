        ...
        <div className="absolute inset-0 z-10 opacity-30 flex flex-col items-stretch pointer-events-none overflow-hidden">
          {[
              ...
              <div key={row.id} className="flex-1 flex w-full overflow-hidden border-b border-white/5">
                <div className={`flex gap-0 w-max ${row.direction}`} style={{ animationDuration: row.speed, animationDelay: row.delay, willChange: 'transform' }}>
        ...
        <div className="w-full flex flex-col lg:flex-row gap-12 md:gap-20 items-start">
          ...
          <main className="w-full lg:flex-1">
            ...
            {currentItems.length > 0 ? (
              <>
                {/* Mobile Friendly Grid: 2 columns on small screens, 3 on XL */}
                  ...
                  {currentItems.map((candidate) => (
                    <Link key={candidate.id} to={`/candidates/${candidate.id}`} className="group">
                      <div className="relative aspect-[3/4] overflow-hidden bg-[#081612] border border-white/5 transition-all duration-1000 group-hover:border-[#d4af37]/40 group-hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.8)]">
                        ...
                        {/* Initials Backdrop (Scaled for grid size) */}
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                          <span className="text-[6rem] md:text-[12rem] font-serif font-black text-white opacity-0 group-hover:opacity-[0.03] transition-all duration-1000 group-hover:scale-150 uppercase select-none">
                        ...
                        <div className="absolute inset-0 z-20 bg-gradient-to-t from-[#0a1f1a] via-[#0a1f1a]/40 to-transparent flex flex-col justify-end p-3 sm:p-6 md:p-10 transition-all duration-700 group-hover:bg-[#0a1f1a]/60">
                            <div className="transform transition-transform duration-700 group-hover:-translate-y-2">
                              ...
                              {/* Bio Reveal (Hidden on small mobile) */}
                        ...
                        {/* Corner Accents */}
                        <div className="absolute top-3 right-3 sm:top-6 sm:right-6 w-4 h-4 sm:w-8 sm:h-8 border-t border-r border-[#d4af37]/0 group-hover:border-[#d4af37]/40 transition-all duration-1000"></div>
                        <div className="absolute bottom-3 left-3 sm:bottom-6 sm:left-6 w-4 h-4 sm:w-8 sm:h-8 border-b border-l border-[#d4af37]/0 group-hover:border-[#d4af37]/40 transition-all duration-1000"></div>
                      </div>
                    </Link>
                  ))}
                </div>
                ...
                {totalPages > 1 && (
                  <div className="mt-12 sm:mt-24 pt-8 sm:pt-12 border-t border-white/5 flex items-center justify-between px-1">
                    <button 
                      ...
                      className="text-[#d4af37] text-[9px] sm:text-[10px] uppercase tracking-[2px] sm:tracking-[4px] font-black disabled:opacity-20 transition-all hover:text-white"
                    >
                      ← Prev
                    </button>
                        ...
                        <button
                          ...
                          className={`w-7 h-7 sm:w-10 sm:h-10 rounded-full text-[9px] sm:text-[10px] font-bold transition-all duration-500 border ${
                            currentPage === i + 1 
                              ...
                              : 'border-white/5 text-stone-600 hover:text-white hover:border-white/20'
                          }`}
                        >
                          {i + 1}
                        </button>
